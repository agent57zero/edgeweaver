...
| 3 | Reflect | today's episodes + last 3 reflections | 1–3 `interpretation` reflections citing ≥2 thought-IDs each; any `self_belief` updates (see below) | Generative-Agents pattern: "What 1–3 higher-level observations follow from today? Cite evidence." Self-belief change → close old row's valid_to, insert new. |
