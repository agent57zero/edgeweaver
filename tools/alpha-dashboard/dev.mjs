// Local runner for the Alpha dashboard: mirrors Vercel's shape (middleware in
// front of static files and api functions) so the gate and the queries verify
// against the real room before any deploy. Dev-only; never deployed (Vercel
// serves public/ and builds api/ only).
//
//   node tools/alpha-dashboard/dev.mjs   ->  http://localhost:8875
//
// Reads EW_ALPHA_DB_URL from avatars/alpha/.env.local. The gate password is a
// fixed local-only value (below) so the browser flow can be exercised; the
// insecure-cookie knob exists because __Host- cookies need https.
...
if (!env.EW_ALPHA_DB_URL) throw new Error("EW_ALPHA_DB_URL missing from avatars/alpha/.env.local");
process.env.EW_ALPHA_DB_URL = env.EW_ALPHA_DB_URL;
process.env.ALPHA_DASH_PASSWORD = "alpha-local-dev-gate-passphrase";
process.env.ALPHA_DASH_DEV_INSECURE_COOKIE = "1";
...
createServer(async (req, res) => {
  try {
    const url = `http://localhost:8875${req.url}`;
    let body;
    if (req.method === "POST") {
      const chunks = [];
  ...
  console.log("alpha-dashboard dev on http://localhost:8875 (password: alpha-local-dev-gate-passphrase)");
});
