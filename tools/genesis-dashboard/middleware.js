// The entire password gate for the Genesis dashboard (Vercel Routing Middleware,
...
//   - FAIL CLOSED: if GENESIS_DASH_PASSWORD is unset, everything is 503.
...
//     real wall is that the API only ever selects Genesis's alan-audience rows.
//   - Unauthenticated /api requests get 401 JSON, never the login HTML.
//
// GENESIS_DASH_DEV_INSECURE_COOKIE=1 is set ONLY by dev.mjs (local http): it
// drops the __Host- prefix and the Secure attribute so the cookie works on
// plain http. Production never sets it.

const COOKIE_MSG = "ew-genesis-dash-cookie-v1";
...
const devInsecure = () => process.env.GENESIS_DASH_DEV_INSECURE_COOKIE === "1";
const cookieName = () => (devInsecure() ? "ew_genesis_dash_dev" : "__Host-ew_genesis_dash");
...
<title>Edgeweaver Genesis</title>
<style>
...
form { border:1px solid rgba(125,125,125,.35); border-top:4px solid #b45309; border-radius:10px; padding:22px 26px; max-width:340px; }
...
button { margin-top:10px; width:100%; font:inherit; padding:8px 10px; border:none; border-radius:8px; background:#b45309; color:#fff; cursor:pointer; }
</style>
</head>
<body>
<form method="post" action="/ew-login">
<h1>Edgeweaver Genesis</h1>
<p>Alan's read-only window into Genesis's room. Enter the password.</p>
${err}
