...
- [ ] Evidence clustering: script finds confirmed lessons/practices cited ≥N times (start
      N=5) via recall traces → candidate amendment themes.
...
| When | What | Owner |
|---|---|---|
| Nightly | Night loop (11 steps) + coherence snapshot | daemon |
| Every wake | Expectations check; budget decrement | daemon |
| Weekly | Index rebuild-from-atoms; spot-check digest; 3Cell (three questions); edge experiment; boundaries.json regen | daemon + Alan |
...
- [ ] 5. Decide: (a) environmental cause → fix and watch; (b) memory corruption → void
      offending run_ids, re-run weekly index; (c) identity erosion → §11 rollback (revert
      last soul merge; archive stays; journal it honestly).
