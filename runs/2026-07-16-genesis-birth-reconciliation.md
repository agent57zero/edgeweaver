...
| Live `agent-memory-api` | An initial function-list/health check on 2026-07-16 found the function and agent-memory tables absent. The unmodified canonical migration first refused cleanly because this OB1 deployment exposes `thoughts` as a view over the bigint-keyed `brain_thoughts` table; that transaction rolled back. The same migration was then applied with guarded compatibility substitutions for that real table and key type, and the API, table reachability, and trust defaults were verified. | Shared writeback infrastructure is healthy, with instruction use false, evidence use true, confirmation required, and review pending by default. No test memory, runtime-host write, or live night-loop preflight is claimed. |
