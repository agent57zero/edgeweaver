...
| **Session-pooler** DB connection string (direct host is IPv6-only; GitHub runners can't reach it) | arm nightly backups (G2) | Supabase → Settings → Database → Connection string → *Session pooler*, then `gh secret set SUPABASE_DB_URL -R agent57zero/edgeweaver-backups`. **BLOCKED 2026-07-06:** the logged-in Supabase account lacks permission to reset the DB password (dashboard: "You need additional permissions to reset the database password"), so the earlier reset never applied and the true password is unknown; the pooler string authenticates as invalid ("password authentication failed"). Pipeline plumbing is verified (secret set + workflow enable + dispatch all succeed on `repo` scope, no `workflow` scope needed). Recommended default: Alan logs in as the org **Owner** (or elevates the account's role), resets the DB password, and re-saves via the local form; or supplies the original creation-time password. Then re-arm (30s). |
