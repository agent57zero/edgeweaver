...
| A3 | ✓ | 2026-07-06 | `verify-mind-server.mjs` PASS (repeatable stub) + a real one-shot subscription bench: synthetic voice exchange via the SUBSCRIPTION Claude backend (`claude -p` / Sonnet; D12 amended 2026-07-06 to pluggable, subscription-default, raw-SDK+credits a config flag for V3). Frozen-prefix cache breakpoint + prefix order, speculative recall, streamed text, rehearsal-tagged voice episode written then voided (0 residue). **Measured subscription TTFT ~3.1s / total ~4.7s** vs the sub-second voice bar (VOICE-STACK §4) - the number the V3 latency decision uses. `voice/{mind-server,claude-backend,prompt-assembly}.mjs` | |
...
| A21 | PARKED | 2026-07-06 | A3/S2 mind server now DONE. A21 (voice W4 hardening: writeback verification, budget hooks, quiet hours) wires into the LIVE W2 voice loop (LiveKit worker + browser client + barge-in), a real-time-audio build that cannot be meaningfully dark-verified without live audio/mic/browser - an arming-phase activity (S7). Its pieces already exist: A3 writeback, A6 voice budget meter, A5 quiet hours. Builds when W2 is stood up (B2/B3 keys verified and ready) | |
