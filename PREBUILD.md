...
| A3 | PARKED | 2026-07-06 | B1 `ant auth login` DONE (OAuth profile active + verified via `ant auth status`, agent57zero@gmail.com, user:inference scope). BUT the Anthropic org has NO API credits: a live `count-tokens` call returns 400 "credit balance is too low". The mind server is nothing but live streaming Claude calls, so its TTFT bench + synthetic exchange cannot run. Needs credits/billing added to the org (console Plans & Billing). Builds as S2 the moment credits land. Design ready (VOICE-STACK.md); voice keys (B2/B3) verified | |
...
| A21 | PARKED | 2026-07-06 | Blocked downstream of A3/S2 (the voice mind server): B1 auth is done, but A3 now waits on Anthropic API credits (see A3), and A21 also needs the W2 voice loop. Voice ear/mouth/transport keys (B2/B3) verified; builds in S7 after S2 + W2 | |
